# 1.データベースの仕組み: データベースは表でデータを管理している。
-- 表のことを「テーブル」縦の列のことを「カラム」横の行のことを「レコード」と呼ぶ



# 2.クエリとは: データを取得するためにデータベースに送る命令の事
...
# 4.特定のデータの取得「WHERE」とは:「どこのレコードのデータを取得するか」を意味する
...
# 5.データ型とは: 「テキストデータ」や「数値データ」、「日付データ」といったように「データの種類」を示すもの。カラムごとにデータ型を決める
...
# 6.比較演算子を用いた特定のデータの取得(WHEREの条件では「＝」の他に比較記号を用いてデータを取得する事が出来る)
SELECT *
FROM purchases
WHERE price >= 1000;
...
# 12.ORDER BYとは: データを並び替える際に用いる。「ORDER BY　並べたいカラム名 並べ方(ASCもしくはDESC);」 ※ASC-昇順、DESC-降順
SELECT *
FROM purchases
ORDER BY price DESC;
...
# 13.LIMITとは:「最大で何件のデータを取得するか」制限を行う際に用いる。「LIMIT 最大で取得するデータの件数」
SELECT *
FROM purchases
LIMIT 5;
